...
const debug = require('debug')('MasterMaker');
const local_fields = require('../rules/fields_master_maker');
...
class MasterMaker extends base_model {
  constructor() {
    super();

    this.id = 'master_maker';
    this.table = 'master_maker';
    this.form_fields = 'seq_id,maker_name,maker_code';
    ...
    this.routes = {
      ...
      get_maker_list: {
        method: 'post', func: 'get_maker_list', path: '/get_maker_list', no_login: false,
      },
    };
    debug('started');
  }
  ...
  get_maker_list(req, res) {
    DataUtil.query(`SELECT maker_code, maker_name FROM ${this.table} `, [], {}, (err, result) => {
      if (err) {
...
module.exports = new MasterMaker();
