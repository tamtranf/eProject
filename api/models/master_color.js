...
const debug = require('debug')('MasterColor');
const local_fields = require('../rules/fields_master_color');
...
class MasterColor extends base_model {
  constructor() {
    super();

    this.id = 'master_color';
    this.table = 'master_color';
    this.form_fields = 'seq_id,color_name,color_code';
    ...
    this.routes = {
      ...
      get_color_list: {
        method: 'post', func: 'get_color_list', path: '/get_color_list', no_login: false,
      },
    };
    debug('started');
  }
  ...
  get_color_list(req, res) {
    DataUtil.query(`SELECT color_code, color_name FROM ${this.table} `, [], {}, (err, result) => {
      if (err) {
...
module.exports = new MasterColor();
