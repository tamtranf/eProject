...
const debug = require('debug')('ChangeHistory');
const local_fields = require('../rules/fields_change_history');
...
class ChangeHistory extends base_model {
  constructor() {
    super();

    this.id = 'change_history';
    this.table = 'change_history';
    this.form_fields = 'seq_id,change_time,user_name,ref_table,ref_id,mode,old_data,new_data';
  ...
  get_options_conditions(req, extend = {}) {
    ...
    if (req.body.api_request_options) {
      options.conditions += ' ref_table = ? AND ref_id = ? ';
      options.params.push(req.body.api_request_options.ref_table, parseInt(req.body.api_request_options.ref_id));
    }
    return options;
  }
