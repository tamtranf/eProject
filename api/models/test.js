...
class TestModel extends BaseModel {
  constructor() {
    ...
    this.routes = {
      test: { method: 'get', func: 'test_func', path: '/', no_login: true },
      test_access: { method: 'get', func: 'test_access', path: '/access', no_login: true },
      test_disk: { method: 'get', func: 'test_disk', path: '/disk', no_login: true },
      test_db: { method: 'get', func: 'test_db', path: '/db', no_login: true },
    };
  }
  get_fields() {
    return [];
  ...
  test_func(req, res) {
    ResponseUtils.response(req, res, { test: 123, constants: constants }, null);
  }
  test_access(req, res) {
    var server_info = "Server Info:<br>";
  ...
  test_disk(req, res) {
    var base_path = path.join(__dirname, '../', 'temp');
    var file_path = path.join(base_path, 'test_file.txt');
    ...
    fs.writeFile(file_path, time, (err) => {
      ...
      fs.readFile(file_path, 'utf8', (err, loaded_data) => {
        if (err) {
  ...
  test_db(req, res) {
    var time = new Date().getTime() + '_str';
    var time_int = new Date().getSeconds();
    DataUtils.query('INSERT INTO master_test (test_field,test_number)VALUES(?,?)', [time, time_int], {}, (err) => {
      if (err) {
        console.log('ERROR at insert', { err });
        return ResponseUtils.response(req, res, {}, err);
      }
      DataUtils.query('SELECT * FROM master_test order by seq_id  desc limit 0,1', [], {}, (err, data) => {
        if (err) {
