...
import { fields } from '../../../api/rules/fields_master_year';

export default {
  name: 'master_year-form',
  data() {
    return {
      local_fields_ref_a: false,
      local_fields_ref_b: false,
      api_name: 'master_year',
      name: 'MasterYearForm',
    };
  },
  routes: [
    {
      path: '/master_year-form/:seq_id',
      name: 'master_year-form',
    },
    {
      path: '/master_year-form',
      name: 'master_year-form-undefined',
    },
  ],
  ...
  computed: {
    ...
    formFieldsSideA() {
      ...
      if (this.local_fields_ref_a === false) {
        this.local_fields_ref_a = this.setFormDefaultFields(['year_name'], fields);
      }
